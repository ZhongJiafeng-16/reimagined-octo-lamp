...
#define CTRL_KEY(k) ((k)&0x1f)  // macro function, set the higer 3 bits to zero.
enum editorKey {
    BACKSPACE = 127,
    ARROW_LEFT = 1000,
    ARROW_RIGHT,
...
void die(const char *s){
    ...
    write(STDIN_FILENO, "\x1b[2J]", 4);  // refresh  terminal screen 
...
void abAppend(struct abuf *ab, const char *s, int len){
    char *newspace = (char*)realloc(ab->b,ab->len + len);

    if(newspace == NULL) return ;
    memcpy(&newspace[ab->len], s ,len);
    ab->b = newspace;
    ab->len += len;
}
